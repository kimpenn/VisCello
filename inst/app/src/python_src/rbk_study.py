...
def get_relevant_study_urls(pb, host, instance_rid):
    ...
    role_table = pb.RNASeq.Application_File_Role.alias('R')
    path = application_file_table.path.\
           filter(application_file_table.Application_Instance.eq(instance_rid)).\
           link(role_table)
    retval = dict()
    for row in path.entities(path.R.Role_Name, path.F.File_URL):
        retval[row.get("Role_Name")] = "https://{h}{p}".format(h=host, p=row.get("File_URL"))
    return(retval)
