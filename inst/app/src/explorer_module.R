...
explorer_server <- function(input, output, session, sclist, useid, cmeta = NULL){
        ...
        fui <- tagList(
            wellPanel(
                fluidRow(
                    ...
                    column(3, selectInput(ns("bp_log_transform_gene"), "Data scale", choices=list("Normalized count"="log2", "Raw count" = "raw")))
                ),
    ...
    output$plot_scalecolor_ui <- renderUI({
        ...
        if(input$proj_colorBy == 'gene.expr') {
            selectInput(ns("log_transform_gene"), "Data scale", choices=list("Normalized count"="log2", "Raw count" = "raw"))
