...
de_server <- function(input, output, session, sclist = NULL, cmeta = NULL, organism = "mmu"){
    ...
    output$hmap_configure_ui <- renderUI({
        ...
        dropdownButton2(inputId=ns("hmap_configure"),
                        ...
                        fluidRow(
                            column(6, numericInput(ns("hmap_limitlow"), "Accessibility z-score cutoff low", value = -2, step=1)),
                            column(6, numericInput(ns("hmap_limithigh"), "Accessibility z-score cutoff high", value = 2, step=1))
                        ),
