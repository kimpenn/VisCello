...
The `Cello` object is an S4 class specifically designed for visualizing subsets of the data - by storing dimension reduction results of (subsets of) cells that are present in the global ExpressionSet, and any local meta information about the cells, such as clustering results. 
...
* `subset_GSE72857_log2norm.txt`: Log2 normalized expression matrix, same dimension as raw matrix. Note that this matrix is obtained using the normalization procedure in monocle [@trapnell2014dynamics], but any other type of normalized data is allowed.
...
`Cello` object is a class specifically designed for visualization of single cell datasets. It allows embedding of multiple dimension reduction results for different subsets of cells. This allows "zoom-in" analysis on subset of cells as well as differential expression analysis on locally defined clusters. The basic structure of `Cello` is as follows:
...
cello <- compute_tsne_cello(eset, cello, use_dim = 30, n_component = 2, perplexity = 30)
